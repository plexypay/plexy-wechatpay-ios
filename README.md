...
2. Use `https://github.com/Plexy/plexy-wechatpay-ios` as the repository URL.
3. Specify the version to be at least `2.0.0`.

:warning: _Please make sure to use Xcode 12.0+ when adding `PlexyWeChatPayInternal` using Swift Package Manager._
...
1. Add `pod 'PlexyWeChatPayInternal'` to your `Podfile`.
2. Run `pod install`.

### Carthage

1. Add `github "plexy/plexy-wechatpay-ios"` to your `Cartfile`.
...
PlexyWeChatPayInternal is just a wrapper around [the original WeChat Pay SDK](https://developers.weixin.qq.com/doc/oplatform/en/Downloads/iOS_Resource.html), with a few minor improvements:
